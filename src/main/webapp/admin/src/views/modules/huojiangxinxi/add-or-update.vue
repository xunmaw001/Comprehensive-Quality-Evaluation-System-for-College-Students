<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zhengshu" label="证书" prop="zhengshu">
          <file-upload
          tip="点击上传证书"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhengshu?ruleForm.zhengshu:''"
          @change="zhengshuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.zhengshu" label="证书" prop="zhengshu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhengshu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiangxiangmingcheng = true;
          ...
          if(o=='zhengshu'){
            this.ruleForm.zhengshu = obj[o];
	    this.ro.zhengshu = true;
            continue;
          }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huojiangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huojiangxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    zhengshuUploadChange(fileUrls) {
	this.ruleForm.zhengshu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
